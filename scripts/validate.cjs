...
for (const file of ['index.html', 'power-pack.js', 'manifest.webmanifest', 'assets/icons/icon-512.png']) {
  if (!serviceWorker.includes(file)) throw new Error(`sw.js does not cache ${file}`);
}

console.log('Validation passed: app shell, scripts, manifest, icons, and service worker are ready.');
